...
def profile():
    ...
    if request.method == "POST":
        ...
        if profile_pic and profile_pic.filename:
            ...
            cur.execute("UPDATE users SET bio=%s, age=%s, profile_pic=%s WHERE username=%s",
                      (bio, age, filename, session["username"]))
        else:
            cur.execute("UPDATE users SET bio=%s, age=%s WHERE username=%s",
                      (bio, age, session["username"]))

        if remove_pic:
            cur.execute("SELECT profile_pic FROM users WHERE username=%s", (session["username"],))
            ...
            if current_pic:
                try:
                    ...
                    pass
            cur.execute("UPDATE users SET profile_pic=NULL WHERE username=%s", (session["username"],))

        conn.commit()

    cur.execute("SELECT bio, age, profile_pic FROM users WHERE username=%s", (session["username"],))
    row = cur.fetchone()
    conn.close()
